...
func set_character(p_role: String, p_race: String, p_gender: String) -> void:
	role = p_role
	race = p_race
	gender = p_gender
	...
	print("PlayerData: Character set to %s %s %s" % [race, gender, role])
...
func load_from_file() -> bool:
	...
	set_character(save_data.get("role", "tank"), save_data.get("race", "human"), save_data.get("gender", "male"))
