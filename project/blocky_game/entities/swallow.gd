...
## Swallow (BirdA) - Ambient peaceful flying bird
## Flies in circles, swoops, occasionally perches
...
var _flight_radius := 8.0
var _flight_height := 15.0  # Fly at medium height

const FLIGHT_DURATION_MIN = 10.0
const FLIGHT_DURATION_MAX = 20.0
const PERCH_DURATION_MIN = 3.0
const PERCH_DURATION_MAX = 8.0
const PERCH_CHANCE = 0.2  # 20% chance to perch after flying
const FLIGHT_ANIM_SPEED = 0.15  # Fast wing flaps
const CIRCLE_SPEED = 0.5  # How fast to circle
const FLIGHT_SPEED = 4.0  # Movement speed
...
func _ready():
	...
	bob_amplitude = 0.2  # Slight bobbing while flying
	bob_frequency = 3.0  # Faster bobbing for bird

	# Call parent ready
	super._ready()

	# Create sprite (start with flight frame 1)
	_sprite = _create_sprite("res://assets/art/animals/birdA_1.png", 0.002)
...
func _handle_flying(delta: float):
	...
	if _flight_anim_timer >= FLIGHT_ANIM_SPEED:
		...
		# Update sprite
		if _sprite:
			var sprite_path = "res://assets/art/animals/birdA_%d.png" % _current_flight_frame
...
func _handle_perched(delta: float):
	...
	if _sprite:
		var texture = load("res://assets/art/animals/birdA_1.png")
...
func _start_flying():
	...
	_flight_center.y = randf_range(10.0, 30.0)  # Vary flight height

	# Random starting angle
	_circle_angle = randf() * TAU

	# Vary circle radius
	_flight_radius = randf_range(5.0, 12.0)
...
func _start_perching():
	...
	if terrain:
		...
		var ray_end = global_position - Vector3(0, 50, 0)
		var hit = vt.raycast(ray_start, (ray_end - ray_start).normalized(), 50.0)
