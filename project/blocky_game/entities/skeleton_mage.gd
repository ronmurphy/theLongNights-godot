...
## Skeleton Mage - Tier 3 RANGED Enemy
## Necromancer that shoots necrotic bolts and can raise fallen enemies
...
const NecroticBolt = preload("res://blocky_game/projectiles/necrotic_bolt.gd")
...
const RAISE_DEAD_COOLDOWN := 10.0  # Can raise dead every 10 seconds


func _ready():
	entity_id = "skeleton_mage"

	var data = EntityBase.load_entity_data("skeleton_mage")
	if not data.is_empty():
		apply_entity_data(data)
	else:
		entity_name = "Skeleton Mage"
		team = Team.ENEMY
		max_hp = 22
		attack_damage = 8
		defense = 14
	...
	_sprite = _create_sprite("res://assets/art/entities/skeleton_mage_ready_pose_enhanced.png", 0.0025)
...
func _process(delta: float):
	...
	# Try to raise dead if cooldown ready (future implementation)
	if _raise_dead_cooldown <= 0:
		# TODO: Implement raise dead ability (spawn zombie_crawler)
	...
	if distance > chase_distance:
		...
		direction.y = 0
		movement_input = direction * movement_speed * 0.4
	else:
		if _attack_timer <= 0:
...
func _perform_ranged_attack():
	...
	print("💀 %s casts necrotic bolt at %s!" % [entity_name, target_name])

	var bolt = Node3D.new()
	bolt.set_script(NecroticBolt)
	...
	if game:
		...
		var time_to_impact = global_position.distance_to(_current_target.global_position) / 20.0
	...
	if _sprite:
		var attack_texture = load("res://assets/art/entities/skeleton_mage_attack_pose_enhanced.png")
		if attack_texture:
			_sprite.texture = attack_texture
			await get_tree().create_timer(0.6).timeout
			if _sprite and is_alive:
				var ready_texture = load("res://assets/art/entities/skeleton_mage_ready_pose_enhanced.png")
				if ready_texture:
