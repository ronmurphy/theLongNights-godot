...
func _ready():
	entity_id = "goblin_shamanka"

	var data = EntityBase.load_entity_data("goblin_shamanka")
	if not data.is_empty():
		apply_entity_data(data)
	else:
		entity_name = "Goblin Shamanka"
		team = Team.ENEMY
		max_hp = 25
		attack_damage = 7
		defense = 13
	...
	_sprite = _create_sprite("res://assets/art/entities/goblin_shamanka_ready_pose_enhanced.png", 0.0025)
...
func _perform_ranged_attack():
	...
	print("🔮 %s casts anguish magic at %s!" % [entity_name, target_name])

	# Create anguish bolt
	var bolt = Node3D.new()
	bolt.set_script(AnguishBolt)
	...
	if game:
		...
		var time_to_impact = global_position.distance_to(_current_target.global_position) / 18.0
	...
	if _sprite:
		var attack_texture = load("res://assets/art/entities/goblin_shamanka_attack_pose_enhanced.png")
		if attack_texture:
			_sprite.texture = attack_texture
			await get_tree().create_timer(0.5).timeout
			if _sprite and is_alive:
				var ready_texture = load("res://assets/art/entities/goblin_shamanka_ready_pose_enhanced.png")
				if ready_texture:
