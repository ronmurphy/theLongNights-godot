...
## Skeleton Archer - Tier 2 RANGED Enemy
## Skeletal marksman that fires bone arrows from distance
...
const BoneArrow = preload("res://blocky_game/projectiles/bone_arrow.gd")
...
func _ready():
	entity_id = "skeleton_archer"

	var data = EntityBase.load_entity_data("skeleton_archer")
	if not data.is_empty():
		apply_entity_data(data)
	else:
		entity_name = "Skeleton Archer"
		team = Team.ENEMY
		max_hp = 10
		attack_damage = 6
		defense = 13
...
func _process(delta: float):
	...
	if distance > chase_distance:
		...
		# Move to preferred shooting range
...
func _perform_ranged_attack():
	...
	if game:
		...
		var time_to_impact = global_position.distance_to(_current_target.global_position) / 25.0
