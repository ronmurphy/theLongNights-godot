...
## Brood Guardian - Tier 2 Tank Enemy
## Protects smaller creatures, high defense, damage reduction
...
func _ready():
	entity_id = "brood_guardian"

	var data = EntityBase.load_entity_data("brood_guardian")
	if not data.is_empty():
		apply_entity_data(data)
	else:
		entity_name = "Brood Guardian"
		team = Team.ENEMY
		max_hp = 28
		attack_damage = 7
		defense = 16
	...
	_sprite = _create_sprite("res://assets/art/entities/brood_guardian_ready_pose_enhanced.png", 0.003)
	_create_health_bar()
	set_collision_box(Vector3(0.8, 1.2, 0.8))  # Larger tank
	_find_best_target()
...
func _perform_attack():
	...
	print("🛡️ %s slams %s for %d damage!" % [entity_name, target_name, attack_damage])

	if _current_target.has_method("take_damage"):
		_current_target.take_damage(attack_damage, self)

	if _sprite:
		var attack_texture = load("res://assets/art/entities/brood_guardian_attack_pose_enhanced.png")
		if attack_texture:
			_sprite.texture = attack_texture
			await get_tree().create_timer(0.4).timeout
			if _sprite and is_alive:
				var ready_texture = load("res://assets/art/entities/brood_guardian_ready_pose_enhanced.png")
				if ready_texture:
