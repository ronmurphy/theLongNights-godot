...
func _use(trans: Transform3D):
	...
	# Spawn ice arrow projectile
	_spawn_ice_arrow(origin, target_pos, direction)


func _spawn_ice_arrow(start_pos: Vector3, target_pos: Vector3, initial_dir: Vector3):
	...
	arrow.initialize(start_pos, target_pos, initial_dir)

	print("Ice bow fired! Target: ", target_pos)
