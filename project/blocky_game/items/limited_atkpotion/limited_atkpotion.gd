...
## Limited ATK Potion - Increases attack bonus by 1 (half of normal potion)
...
func use(trans: Transform3D, inv_item_or_count = 1):
	"""Use the Limited ATK potion - show choice dialog"""
...
func _use(trans: Transform3D, inv_item_or_count):
	...
	if not dialog:
		push_error("PotionChoiceDialog not found! Cannot use Limited ATK potion.")
		return
	...
	# Show the dialog
	dialog.show_potion_choice("atk", "Attack", BONUS_AMOUNT)
...
func _on_choice_made(target: String, inv_item_or_count):
	...
	# Handle choice
	if target == "cancel":
		print("Limited ATK Potion use cancelled.")
		return

	# Apply the bonus
	if target == "player":
		PlayerData.bonus_attack += BONUS_AMOUNT
		PlayerData.save_to_file()
		print("💊 %s gained +%d Attack! (Total bonus: +%d)" % [
			PlayerData.player_name if PlayerData.player_name != "" else "Player",
			BONUS_AMOUNT,
			PlayerData.bonus_attack
		])
	elif target == "companion":
		CompanionManager.companion_bonus_attack += BONUS_AMOUNT
		...
		print("💊 %s gained +%d Attack! (Total bonus: +%d)" % [
			companion_name,
			BONUS_AMOUNT,
			CompanionManager.companion_bonus_attack
		])
