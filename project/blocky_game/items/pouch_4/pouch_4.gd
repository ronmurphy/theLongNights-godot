...
## Pouch Tier 4 - Epic loot bag
## Drops from Tier 4 enemies
## Contains: 2 skyshards + epic weapons/rare items

const SERVER_PEER_ID = 1
const InventoryItem = preload("../../player/inventory_item.gd")

# Loot table - Epic weapons and rare items
const LOOT_TABLE = {
	# Epic/Rare weapons (weight 10-12)
	"ice_bow": 12,
	"fire_staff": 12,
	"blade_of_pursuit": 10,
	"ring_of_thorns": 10,
	...
	# Valuable materials - always ore (weight 8-10)
	"gold_ore": 10,
	"iron_ore": 8,
}
...
func _use(trans: Transform3D, inv_item_or_count):
	...
	# Always give 2 skyshards
	var skyshard_item = _find_item_by_name("skyshard")
	if skyshard_item:
		_add_item_to_inventory(inventory, skyshard_item.base_info.id, 2)
		print("🔥 Epic Pouch opened! +2 Skyshards")
