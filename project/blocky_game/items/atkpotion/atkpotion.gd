...
func use(trans: Transform3D, inv_item_or_count = 1):
	"""Use the Attack potion - show choice dialog"""
...
func _use(trans: Transform3D, inv_item_or_count):
	...
	if not dialog:
		push_error("PotionChoiceDialog not found! Cannot use Attack potion.")
		return
	...
	# Show the dialog
	dialog.show_potion_choice("attack", "Attack", BONUS_AMOUNT)
...
func _on_choice_made(target: String, inv_item_or_count):
	...
	# Handle choice
	if target == "cancel":
		print("Attack Potion use cancelled.")
		return

	# Apply the bonus
	if target == "player":
		PlayerData.bonus_attack += BONUS_AMOUNT
		PlayerData.attack_bonus += BONUS_AMOUNT  # Update current attack too
		PlayerData.save_to_file()
		print("⚔️ %s gained +%d Attack! (Total bonus: +%d)" % [
