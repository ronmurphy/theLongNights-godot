...
## Limited DEF Potion - Increases defense by 1 (half of normal potion)
...
func use(trans: Transform3D, inv_item_or_count = 1):
	"""Use the Limited DEF potion - show choice dialog"""
...
func _use(trans: Transform3D, inv_item_or_count):
	...
	if not dialog:
		push_error("PotionChoiceDialog not found! Cannot use Limited DEF potion.")
		return
	...
	# Show the dialog
	dialog.show_potion_choice("def", "Defense", BONUS_AMOUNT)
...
func _on_choice_made(target: String, inv_item_or_count):
	...
	# Handle choice
	if target == "cancel":
		print("Limited DEF Potion use cancelled.")
		return
	...
	if target == "player":
		...
		print("💊 %s gained +%d Defense! (Total bonus: +%d)" % [
