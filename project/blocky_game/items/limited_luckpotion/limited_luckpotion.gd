...
## Limited LUCK Potion - Increases luck by 1 (half of normal potion)
...
func use(trans: Transform3D, inv_item_or_count = 1):
	"""Use the Limited LUCK potion - show choice dialog"""
...
func _use(trans: Transform3D, inv_item_or_count):
	...
	if not dialog:
		push_error("PotionChoiceDialog not found! Cannot use Limited LUCK potion.")
		return
...
func _on_choice_made(target: String, inv_item_or_count):
	...
	# Handle choice
	if target == "cancel":
		print("Limited LUCK Potion use cancelled.")
		return
