...
## Pouch Tier 5 - Legendary loot bag
## Drops from Tier 5 enemies (5% chance)
## Contains: 3 skyshards + guaranteed rare/epic weapon + bonus ore

const SERVER_PEER_ID = 1
const InventoryItem = preload("../../player/inventory_item.gd")

# Loot table - Only the best weapons
const LOOT_TABLE = {
	# Epic weapons only (weight 15-20)
	"ice_bow": 20,
	"fire_staff": 20,
	"blade_of_pursuit": 18,
	"ring_of_thorns": 15,
...
func _use(trans: Transform3D, inv_item_or_count):
	...
	# Always give 3 skyshards
	var skyshard_item = _find_item_by_name("skyshard")
	if skyshard_item:
		_add_item_to_inventory(inventory, skyshard_item.base_info.id, 3)
		print("🌟 LEGENDARY Pouch opened! +3 Skyshards")

	# Guaranteed rare weapon
	var weapon_name = _roll_loot_table()
	var weapon_item = _find_item_by_name(weapon_name)
	if weapon_item:
		_add_item_to_inventory(inventory, weapon_item.base_info.id, 1)
		print("   +1 %s ⚔️" % weapon_name)

	# BONUS: Also give gold ore (8-12)
	var gold_item = _find_item_by_name("gold_ore")
	if gold_item:
		var gold_count = randi_range(8, 12)
		_add_item_to_inventory(inventory, gold_item.base_info.id, gold_count)
		print("   +%d gold_ore 💰" % gold_count)
