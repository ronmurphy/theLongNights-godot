...
## Limited HP Potion - Increases max HP by 5 (half of normal potion)
...
func use(trans: Transform3D, inv_item_or_count = 1):
	"""Use the Limited HP potion - show choice dialog"""
...
func _use(trans: Transform3D, inv_item_or_count):
	...
	if not dialog:
		push_error("PotionChoiceDialog not found! Cannot use Limited HP potion.")
		return
...
func _on_choice_made(target: String, inv_item_or_count):
	...
	# Handle choice
	if target == "cancel":
		print("Limited HP Potion use cancelled.")
		return
